@extends('layouts/admin')

@section('plugins.TempusDominusBs4', true)

@section('content_header')
<h3>Editar Siaf</h3>
@stop

@section('content')

<div class="row">
  <div class="col-12 col-lg-11">
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">Editar Siaf:</h3>
      </div>
      <div class="card-body">
        @include('admin.partials.validation-errors')
        <form id="registerForm" action="{{ route('documentSiafs.update', $documentSiaf) }}" method="POST" >
          @csrf
          @method('patch')
          @include('admin.document_siaf._form_step_2')
          <div class="ln_solid"></div>
          <div class="form-group row mt-4">
            <div class="col-lg-12 col-sm-4 col-xs-12 col-md-offset-2">
              @if( ($documentSiaf->status ?? '') !== 3)<button type="submit" class="btn btn-success">Guardar</button>@endif
              <a href="{{ session('previous_url_document_siaf') }}" class="btn btn-primary">Cancelar</a>
            </div>
          </div>
        </form>
      </div>
    </div>
  </div>
</div>

@include('admin.document_siaf._form_js')

@stop