...
<div class="row">
  <div class="col-md-12 d-flex flex-column flex-md-row justify-content-between align-items-md-center">
    <h1 class="mb-2 mb-sm-0">Encargos</h1>
    <div class="d-flex flex-column flex-sm-row">
      <a href="{{ route('orderRegisters.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Aperturar Encargos</a>
    </div>
  </div>
</div>
@stop
...
<div class="card">
  ...
  <div class="card-body table-responsive">
    <table class="table table-striped table-hover">
      <caption>Listado de encargos en total: {{  $items->total() }}</caption>
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="align-middle">
            <div class="btn-group">
              <a href="{{ route('orderRegisterDetails.index', $item->id) }}" class="btn btn-info"><span class="fa fa-arrow-right"></span></a>
              @if($item?->closed != 1) <x-adminlte-button type="button" theme="danger" icon="fa fa-times"  data-toggle="modal" data-id="{{ $item->id }}" data-target="#onCloseOrderRegister" /> @endif
...
@include('admin.order_register._modal_form_close')
...
<script type="text/javascript">
  document.querySelectorAll(".btn-onPrint").forEach(element =>
    element.addEventListener("click", function() {
      ...
      const objData = {
        order_register_id: element.dataset.id
      };

      let queryParams = new URLSearchParams(objData).toString();

      fetch(`{{ route("reports.orderRegisterDetails") }}?${queryParams}`)
        .then(response => {
          if (!response.ok) {
        ...
        .then(blob => {
          ...
          a.download = `excelEncargo_${number}.xlsx`;
  ...
  $('#onCloseOrderRegister').on('show.bs.modal', function (event) {
    ...
    fetch("{{ route('orderRegisters.show', ':id') }}".replace(':id', dataId))
      .then(response => response.json())
      .then(data => {
        formulario.find('#order_register_id').val(data.cash_register_id);
