...
<script type="text/javascript">
  ...
  function updateTable(records) {
      ...
      records.forEach( (record, index) => {
          ...
          newRow.innerHTML = `
              ...
              @if( (isset($orderRegister) && !$orderRegister?->closed)  )
  ...
  function onLoad(){
    ...
    if("{{ isset($orderRegisterDetail) }}"){
      let valueOriginal = "{{ old('issue_description', $orderRegisterDetail->issue_description ?? '') }}";
    ...
    let formDisable = "{{ old('view', $orderRegisterDetail->view ?? false) }}";
    if (formDisable == true) {
