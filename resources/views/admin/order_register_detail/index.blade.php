...
<div class="row">
  <div class="col-md-12 d-flex flex-column flex-md-row justify-content-between align-items-md-center">
    <h1 class="mb-2 mb-sm-0">Encargos detalle</h1>
    <div class="d-flex flex-column flex-sm-row">
      <a href="{{ route('orderRegisterDetails.create', ['orderRegister' => $orderRegister->id]) }}" class="btn btn-primary"><i class="fas fa-plus"></i> Nuevo registro</a>
    </div>
  </div>
</div>
@stop
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="align-middle">
            <div class="btn-group">
              <a href="{{ route('orderRegisterDetails.show', ['orderRegister' => $orderRegister->id, 'orderRegisterDetail' => $item->id]) }}" class="btn btn-info"><span class="fa fa-eye"></span></a>
              @if(!$orderRegister?->closed)
                <a href="{{ route('orderRegisterDetails.edit', ['orderRegister' => $orderRegister->id, 'orderRegisterDetail' => $item->id]) }}" class="btn btn-success"><span class="fa fa-edit"></span></a>
    ...
    element.addEventListener("click", function() {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: 'DELETE',
            url: "{{ route('orderRegisterDetails.destroy', [$orderRegister, ':id']) }}".replace(':id', element.dataset.id),
            data: {
              "_token": "{{ csrf_token() }}"
            },
