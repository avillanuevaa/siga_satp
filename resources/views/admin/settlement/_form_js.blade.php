...
<script type="text/javascript">
  ...
  const recordsArrayInput = document.getElementById('settlementClassifier');

  const approvalSettlement = "{{ old('approval', $settlement->approval ?? '') }}";
  const request_id = document.getElementById('request_id').value;

  if (approvalSettlement === 1 || !request_id ) {
  ...
  function updateTable(records) {
      ...
      records.forEach( (record, index) => {
          ...
          newRow.innerHTML = `
              ...
              @if( (isset($settlement) && $settlement?->approval) !== 1)
  ...
  const settlementClassifier = JSON.parse(recordsArrayInput.value); //JSON.parse(recordsArrayInput.value);
  settlementClassifier.forEach(record => {
      recordsArray.push({
