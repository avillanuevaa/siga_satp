...
<div class="row">
  <div class="col-md-12 d-flex flex-column flex-md-row justify-content-between align-items-md-center">
    <h1 class="mb-2 mb-sm-0">Liquidación</h1>
    <div class="d-flex flex-column flex-sm-row">
      <a href="{{ route('settlements.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Nueva Liquidación</a>
    </div>
  </div>
</div>
@stop
...
<div class="card">
  ...
  <div class="card-body">
    @include('admin.settlement._search')
  </div>
</div>
<div class="card">
  <div class="card-header">
  ...
  <div class="card-body table-responsive">
    <table class="table table-striped table-hover">
      <caption>Listado de liquidaciones en total: {{  $items->total() }}</caption>
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="text-center align-middle">{{ \Carbon\Carbon::parse($item->requestFile?->request_date)->format('d/m/Y') }}</td>
          ...
          <td class="align-middle">
            <div class="btn-group">
              <a href="{{ route('settlements.edit', $item->id) }}" class="btn btn-info"><span class="fa fa-edit"></span></a>
...
<script type="text/javascript">
  document.querySelectorAll(".btn-approval").forEach(element =>
    element.addEventListener("click", function() {
      Swal.fire({
        title: 'Desea aprobar la liquidación?',
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: 'POST',
            url: "{{ route('settlements.updateApproval', ':id' ) }}".replace(':id', element.dataset.id),
            data: {
              "_token": "{{ csrf_token() }}"
            },
  ...
  document.querySelectorAll(".btn-onPrint").forEach(element =>
    element.addEventListener("click", function() {
      ...
      const objData = {
        settlement_id: element.dataset.id
      };

      let queryParams = new URLSearchParams(objData).toString();

      fetch(`{{ route("reports.settlementFileDetails") }}?${queryParams}`)
        .then(response => {
          if (!response.ok) {
        ...
        .then(blob => {
          ...
          a.download = `excelLiquidacion_${type_name}_${correlative}.xlsx`;
