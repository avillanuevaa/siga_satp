...
<div class="row">
  <div class="col-md-12 d-flex flex-column flex-md-row justify-content-between align-items-md-center">
    <h1 class="mb-2 mb-sm-0">Viaticos detalle</h1>
    <div class="d-flex flex-column flex-sm-row">
      <a href="{{ route('viaticRegisterDetails.create', ['viaticRegister' => $viaticRegister->id]) }}" class="btn btn-primary mb-2 mb-sm-0 mr-sm-2"><i class="fas fa-plus"></i> Nuevo registro</a>
      <x-adminlte-button type="button" class="mb-2 mb-sm-0 mr-sm-2" label="Declaración jurada" theme="secondary" icon="fas fa-plus"  data-toggle="modal" data-target="#registerAffidavit" />
      <x-adminlte-button type="button" class="mb-2 mb-sm-0 mr-sm-2" label="Informe de comisión de servicios" theme="secondary" icon="fas fa-plus"  data-toggle="modal" data-target="#registerServiceCommission" />
    </div>
  </div>
</div>
@stop
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="align-middle">
            <div class="btn-group">
              <a href="{{ route('viaticRegisterDetails.show', ['viaticRegister' => $viaticRegister->id, 'viaticRegisterDetail' => $item->id]) }}" class="btn btn-info"><span class="fa fa-eye"></span></a>
              @if(!$viaticRegister?->closed)
                <a href="{{ route('viaticRegisterDetails.edit', ['viaticRegister' => $viaticRegister->id, 'viaticRegisterDetail' => $item->id]) }}" class="btn btn-success"><span class="fa fa-edit"></span></a>
...
@include('admin.viatic_register_detail._modal_register_affidavit_form')
@include('admin.viatic_register_detail._modal_register_service_commission_form')
    ...
    element.addEventListener("click", function() {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: 'DELETE',
            url: "{{ route('viaticRegisterDetails.destroy', [$viaticRegister, ':id']) }}".replace(':id', element.dataset.id),
            data: {
              "_token": "{{ csrf_token() }}"
            },
